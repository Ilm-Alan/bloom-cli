...
from bloom.cli.textual_ui.terminal_theme import TERMINAL_THEME_NAME
...
class ConfigApp(Container):
    ...
    def __init__(
        self, config: BloomConfig, *, has_terminal_theme: bool = False
    ) -> None:
        ...
        self._model_display_names: dict[str, str] = {
            m.alias: m.display_name for m in self.config.models
        }
    ...
    def _get_raw_value(self, setting: SettingDefinition) -> str:
        """Raw option value (alias) for cycling logic."""
        key = setting["key"]
    ...
    def _format_for_display(self, key: str, value: str) -> str:
        """Map a raw value to a human-readable label."""
        if key == "active_model":
            return self._model_display_names.get(value, value)
        return value
    ...
    def _update_display(self) -> None:
        for i, (setting, widget) in enumerate(
            ...
            value = self._format_for_display(
                setting["key"], self._get_raw_value(setting)
            )
    ...
    def action_toggle_setting(self) -> None:
        ...
        key: str = setting["key"]
        current: str = self._get_raw_value(setting)
