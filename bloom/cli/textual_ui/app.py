...
from bloom import __version__ as CORE_VERSION
from bloom.cli.clipboard import copy_selection_to_clipboard
from bloom.cli.commands import CommandRegistry
from bloom.cli.terminal_setup import setup_terminal
from bloom.cli.textual_ui.handlers.event_handler import EventHandler
from bloom.cli.textual_ui.terminal_theme import (
    TERMINAL_THEME_NAME,
    capture_terminal_theme,
)
from bloom.cli.textual_ui.widgets.approval_app import ApprovalApp
from bloom.cli.textual_ui.widgets.banner.banner import Banner
from bloom.cli.textual_ui.widgets.chat_input import ChatInputContainer
from bloom.cli.textual_ui.widgets.compact import CompactMessage
from bloom.cli.textual_ui.widgets.config_app import ConfigApp
from bloom.cli.textual_ui.widgets.context_progress import ContextProgress, TokenState
from bloom.cli.textual_ui.widgets.load_more import HistoryLoadMoreRequested
from bloom.cli.textual_ui.widgets.loading import LoadingWidget, paused_timer
from bloom.cli.textual_ui.widgets.messages import (
    AssistantMessage,
    BashOutputMessage,
    ErrorMessage,
    InterruptMessage,
...
from bloom.cli.textual_ui.widgets.path_display import PathDisplay
from bloom.cli.textual_ui.widgets.question_app import QuestionApp
from bloom.cli.textual_ui.widgets.tools import ToolCallMessage, ToolResultMessage
...
from bloom.cli.update_notifier import (
...
from bloom.cli.update_notifier.update import do_update
from bloom.core.agent_loop import AgentLoop
from bloom.core.agents import AgentProfile
from bloom.core.autocompletion.path_prompt_adapter import render_path_prompt
from bloom.core.config import BloomConfig
from bloom.core.paths.config_paths import HISTORY_FILE
from bloom.core.session.session_loader import SessionLoader
from bloom.core.tools.base import ToolPermission
from bloom.core.tools.builtins.ask_user_question import (
    AskUserQuestionArgs,
    AskUserQuestionResult,
)
from bloom.core.types import AgentStats, ApprovalResponse, LLMMessage, RateLimitError
from bloom.core.utils import (
    CancellationReason,
...
class BloomApp(App):  # noqa: PLR0904
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._banner_version = banner_version or CORE_VERSION
    ...
    @property
    def config(self) -> BloomConfig:
        return self.agent_loop.config

    def compose(self) -> ComposeResult:
        with ChatScroll(id="chat"):
            self._banner = Banner(
                self.config, self.agent_loop.skill_manager, version=self._banner_version
            )
            yield self._banner
    ...
    async def on_mount(self) -> None:
        ...
        def update_context_progress(stats: AgentStats) -> None:
            context_progress.tokens = TokenState(
                max_tokens=self.config.effective_context_limit,
    ...
    async def on_config_app_config_closed(
        ...
        if message.changes:
            BloomConfig.save_updates(message.changes)
...
def _print_session_resume_message(session_id: str | None) -> None:
    ...
    print()
    print("To continue this session, run: bloom --continue")
    print(f"Or: bloom --resume {session_id}")
