...
from bloom.core.paths.global_paths import GLOBAL_ENV_FILE
from bloom.setup.onboarding.base import OnboardingScreen

PROVIDER_HELP = {
    "pollinations": ("https://enter.pollinations.ai", "Pollinations Dashboard")
}
CONFIG_DOCS_URL = "https://github.com/Ilm-Alan/bloom-cli#configuration"

_KEY_VALIDATION_URL = "https://gen.pollinations.ai/account/key"
...
class ApiKeyScreen(OnboardingScreen):
    ...
    def __init__(self) -> None:
        super().__init__()
        config = BloomConfig.model_construct()
    ...
    def _compose_config_docs(self) -> ComposeResult:
        yield Static("[dim]Learn more about Bloom configuration:[/]")
        yield Horizontal(
            NoMarkupStatic("→ ", classes="link-chevron"),
    ...
    def on_input_submitted(self, event: Input.Submitted) -> None:
        if event.validation_result and event.validation_result.is_valid:
            self._verify_and_save(event.value.strip())

    def _verify_and_save(self, api_key: str) -> None:
        feedback = self.query_one("#feedback", NoMarkupStatic)
        feedback.update("Verifying API key...")
        feedback.remove_class("error", "success")
        self.input_widget.disabled = True
        self.run_worker(self._do_verify(api_key), exclusive=True)

    async def _do_verify(self, api_key: str) -> None:
        feedback = self.query_one("#feedback", NoMarkupStatic)
        input_box = self.query_one("#input-box")
        ...
        try:
            async with httpx.AsyncClient(timeout=10.0) as client:
                resp = await client.get(
                    _KEY_VALIDATION_URL, headers={"Authorization": f"Bearer {api_key}"}
                )
            if resp.status_code == _HTTP_UNAUTHORIZED:
                feedback.update("Invalid API key. Please check and try again.")
                feedback.add_class("error")
            ...
            # Network issue — accept the key anyway, they can fix later
            pass
