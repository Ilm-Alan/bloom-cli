...
from bloom.core.paths.global_paths import BLOOM_HOME, GlobalPath
from bloom.core.trusted_folders import trusted_folders_manager
...
def _resolve_config_path(basename: str, type: Literal["file", "dir"]) -> Path:
    ...
    if not is_folder_trusted:
        return BLOOM_HOME.path / basename
    if type == "file":
        if (candidate := cwd / ".bloom" / basename).is_file():
            return candidate
    elif type == "dir":
        if (candidate := cwd / ".bloom" / basename).is_dir():
            return candidate
    return BLOOM_HOME.path / basename
...
def resolve_local_tools_dir(dir: Path) -> Path | None:
    if not trusted_folders_manager.is_trusted(dir):
        return None
    if (candidate := dir / ".bloom" / "tools").is_dir():
        return candidate
    return None
...
def resolve_local_skills_dir(dir: Path) -> Path | None:
    if not trusted_folders_manager.is_trusted(dir):
        return None
    if (candidate := dir / ".bloom" / "skills").is_dir():
        return candidate
    return None
...
def resolve_local_agents_dir(dir: Path) -> Path | None:
    if not trusted_folders_manager.is_trusted(dir):
        return None
    if (candidate := dir / ".bloom" / "agents").is_dir():
        return candidate
    return None
...
HISTORY_FILE = ConfigPath(lambda: _resolve_config_path("bloomhistory", "file"))
