...
from bloom.core.paths.config_paths import CONFIG_DIR, CONFIG_FILE, PROMPTS_DIR
from bloom.core.paths.global_paths import (
    GLOBAL_ENV_FILE,
    GLOBAL_PROMPTS_DIR,
    SESSION_LOG_DIR,
)
from bloom.core.prompts import SystemPrompt
from bloom.core.tools.base import BaseToolConfig
...
class ModelConfig(BaseModel):
    ...
    context_length: int = 0  # 0 = unknown, use auto_compact_threshold fallback
    ...
    def display_name(self) -> str:
        ...
        return self.description or self.alias


DEFAULT_POLLINATIONS_API_KEY = "POLLINATIONS_API_KEY"
...
DEFAULT_PROVIDERS = [
    ProviderConfig(
        ...
        api_key_env_var=DEFAULT_POLLINATIONS_API_KEY,
    )
]
...
class BloomConfig(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        env_prefix="BLOOM_", case_sensitive=False, extra="ignore"
    )
    ...
    def effective_context_limit(self) -> int:
        """Context token limit for the active model (for compaction and UI)."""
        try:
    ...
    def settings_customise_sources(
        ...
        into os.environ for use by providers. Only BLOOM_* prefixed environment
    ...
    def _check_api_key(self) -> BloomConfig:
        try:
    ...
    @model_validator(mode="after")
    def _check_api_backend_compatibility(self) -> BloomConfig:
        return self
    ...
    @model_validator(mode="after")
    def _validate_model_uniqueness(self) -> BloomConfig:
    ...
    @model_validator(mode="after")
    def _check_system_prompt(self) -> BloomConfig:
    ...
    @classmethod
    def load(cls, **overrides: Any) -> BloomConfig:
        cls._migrate()
        config = cls(**(overrides or {}))
        config._merge_discovered_models()
        return config

    def _merge_discovered_models(self) -> None:
        """Merge dynamically discovered models from Pollinations v1 API."""
        try:
            from bloom.core.llm.model_discovery import fetch_models, to_model_configs

            discovered = fetch_models()
            if not discovered:
                return
            new_configs = to_model_configs(discovered)
            existing_aliases = {m.alias for m in self.models}
            for mc in new_configs:
                if mc.alias not in existing_aliases:
                    self.models.append(mc)
                    existing_aliases.add(mc.alias)
        except Exception:
            pass
    ...
    def create_default(cls) -> dict[str, Any]:
        ...
        from bloom.core.tools.manager import ToolManager
