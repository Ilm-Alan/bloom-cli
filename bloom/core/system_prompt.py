...
from bloom.core.trusted_folders import TRUSTABLE_FILENAMES, trusted_folders_manager
from bloom.core.utils import is_dangerous_directory, is_windows

if TYPE_CHECKING:
    from bloom.core.agents import AgentManager
    from bloom.core.config import BloomConfig, ProjectContextConfig
...
def _add_commit_signature() -> str:
    return (
        ...
        "It will always be suffixed with a line telling it was generated by Bloom with the appropriate co-authoring information.\n"
        "The format you will always uses is the following heredoc.\n\n"
        "```bash\n"
        "git commit -m <Commit message here>\n\n"
        "Generated by Bloom.\n"
        "Co-Authored-By: Bloom <bloom@pollinations.ai>\n"
        "```"
    )
...
def get_universal_system_prompt(
    ...
    if config.include_model_info:
        try:
            model_label = config.get_active_model().display_name
        except ValueError:
            model_label = config.active_model
        sections.append(f"Your model name is: `{model_label}`")
