...
from bloom.core.paths.config_paths import resolve_local_tools_dir
from bloom.core.paths.global_paths import DEFAULT_TOOL_DIR, GLOBAL_TOOLS_DIR
from bloom.core.tools.base import BaseTool, BaseToolConfig
from bloom.core.tools.mcp import (
    RemoteTool,
...
from bloom.core.utils import name_matches, run_sync

logger = getLogger("bloom")

if TYPE_CHECKING:
    from bloom.core.config import BloomConfig, MCPHttp, MCPStdio, MCPStreamableHttp


def _try_canonical_module_name(path: Path) -> str | None:
    """Extract canonical module name for bloom package files.
    ...
    try:
        bloom_idx = parts.index("bloom")
    except ValueError:
        return None

    if bloom_idx + 1 >= len(parts):
        return None

    module_parts = [p.removesuffix(".py") for p in parts[bloom_idx:]]
    return ".".join(module_parts)


def _compute_module_name(path: Path) -> str:
    """Return canonical module name for bloom files, hash-based synthetic name otherwise."""
    ...
    return f"bloom_tools_discovered_{stem}_{path_hash}"
...
class ToolManager:
    ...
    def __init__(self, config_getter: Callable[[], BloomConfig]) -> None:
    ...
    @property
    def _config(self) -> BloomConfig:
        return self._config_getter()

    @staticmethod
    def _compute_search_paths(config: BloomConfig) -> list[Path]:
