...
class AgentProfile:
    ...
    def apply_to_config(self, base: BloomConfig) -> BloomConfig:
        ...
        for key, value in self.overrides.items():
            ...
            if key == "tools" and isinstance(value, dict):
                merged_tools = dict(current or {})
                for k, v in value.items():
                    merged_tools[k] = (
                        BaseToolConfig.model_validate(v) if isinstance(v, dict) else v
                    )
                update[key] = merged_tools
