...
from bloom.core.config import BloomConfig, ModelConfig
...
@pytest.fixture
def acp_agent_loop(backend) -> BloomAcpAgentLoop:
    ...
    BloomConfig.dump_config(config.model_dump())
    ...
    patch("bloom.acp.acp_agent_loop.AgentLoop", side_effect=PatchedAgentLoop).start()
...
class TestACPSetModel:
    @pytest.mark.asyncio
    async def test_set_model_success(self, acp_agent_loop: BloomAcpAgentLoop) -> None:
    ...
    async def test_set_model_saves_to_config(
        ...
        with patch("bloom.acp.acp_agent_loop.BloomConfig.save_updates") as mock_save:
            response = await acp_agent_loop.set_session_model(
                session_id=session_id, model_id="devstral-small"
            )
