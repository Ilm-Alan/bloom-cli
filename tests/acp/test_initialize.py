...
class TestACPInitialize:
    @pytest.mark.asyncio
    async def test_initialize(self, acp_agent_loop: BloomAcpAgentLoop) -> None:
        ...
        assert response.agent_info == Implementation(
            name="@ilm-alan/bloom-cli", title="Bloom", version="0.1.2"
        )
    ...
    async def test_initialize_with_terminal_auth(
        ...
        assert auth_method.id == "bloom-setup"
        assert auth_method.name == "Register your API Key"
        assert auth_method.description == "Register your API Key inside Bloom"
        ...
        assert terminal_auth_meta["label"] == "Bloom Setup"
