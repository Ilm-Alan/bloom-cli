...
from bloom.acp.utils import ToolOption
from bloom.core.types import FunctionCall, ToolCall
...
def _create_bloom_home_dir(tmp_path: Path, *sections: dict[str, Any]) -> Path:
    """Create a temporary bloom home directory with a minimal config file."""
    bloom_home = tmp_path / ".bloom"
    bloom_home.mkdir()

    config_file = bloom_home / "config.toml"
    ...
    trusted_folters_file = bloom_home / "trusted_folders.toml"
...
@pytest.fixture
def bloom_home_dir(tmp_path: Path) -> Path:
    """Create a temporary bloom home directory with a minimal config file."""
    return _create_bloom_home_dir(tmp_path)


@pytest.fixture
def bloom_home_grep_ask(tmp_path: Path) -> Path:
    """Create a temporary bloom home directory with grep configured to ask permission."""
    return _create_bloom_home_dir(tmp_path, {"tools": {"grep": {"permission": "ask"}}})
...
async def get_acp_agent_loop_process(
    mock_env: dict[str, str], bloom_home: Path
    ...
    env.update(mock_env)
    env["POLLINATIONS_API_KEY"] = "mock"
    env["BLOOM_HOME"] = str(bloom_home)
...
class TestSessionManagement:
    @pytest.mark.asyncio
    async def test_multiple_sessions_unique_ids(self, bloom_home_dir: Path) -> None:
        mock_env = get_mocking_env(mock_chunks=[mock_llm_chunk() for _ in range(3)])
        async for process in get_acp_agent_loop_process(
            mock_env=mock_env, bloom_home=bloom_home_dir
        ):
            await send_json_rpc(
                process,
...
class TestSessionUpdates:
    ...
    @pytest.mark.asyncio
    async def test_tool_call_update_structure(self, bloom_home_dir: Path) -> None:
...
class TestToolCallStructure:
    ...
    async def test_tool_call_request_permission_structure(
        ...
        async for process in get_acp_agent_loop_process(
            mock_env=mock_env, bloom_home=bloom_home_grep_ask
        ):
            session_id = await initialize_session(process)
            await send_json_rpc(
                process,
