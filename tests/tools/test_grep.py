...
from bloom.core.tools.base import ToolError
from bloom.core.tools.builtins.grep import (
    Grep,
    GrepArgs,
    GrepBackend,
    GrepState,
...
@pytest.mark.asyncio
async def test_respects_bloomignore_file(grep, tmp_path):
    (tmp_path / ".bloomignore").write_text("custom_dir/\n*.tmp\n")
...
@pytest.mark.asyncio
async def test_ignores_comments_in_bloomignore(grep, tmp_path):
    (tmp_path / ".bloomignore").write_text("# comment\npattern/\n# another comment\n")
...
class TestGnuGrepBackend:
    ...
    @pytest.mark.asyncio
    async def test_respects_bloomignore_file(self, grep_gnu_only, tmp_path):
        (tmp_path / ".bloomignore").write_text("custom_dir/\n*.tmp\n")
        custom_dir = tmp_path / "custom_dir"
        custom_dir.mkdir()
        (custom_dir / "excluded.py").write_text("match\n")
