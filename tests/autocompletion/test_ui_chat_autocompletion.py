...
from bloom.cli.textual_ui.widgets.chat_input.completion_popup import CompletionPopup
from bloom.cli.textual_ui.widgets.chat_input.container import ChatInputContainer


@pytest.mark.asyncio
async def test_popup_appears_with_matching_suggestions(bloom_app: BloomApp) -> None:
    async with bloom_app.run_test() as pilot:
        chat_input = bloom_app.query_one(ChatInputContainer)
        popup = bloom_app.query_one(CompletionPopup)
...
@pytest.mark.asyncio
async def test_popup_hides_when_input_cleared(bloom_app: BloomApp) -> None:
    async with bloom_app.run_test() as pilot:
        popup = bloom_app.query_one(CompletionPopup)
...
@pytest.mark.asyncio
async def test_arrow_navigation_cycles_through_suggestions(bloom_app: BloomApp) -> None:
    async with bloom_app.run_test() as pilot:
        popup = bloom_app.query_one(CompletionPopup)
...
async def test_pressing_enter_submits_selected_command_and_hides_popup(
    ...
    async with bloom_app.run_test() as pilot:
        ...
        assert popup.styles.display == "none"
        message = bloom_app.query_one(".user-command-message")
...
def file_tree(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    (tmp_path / "bloom" / "acp" / "entrypoint.py").write_text("", encoding="utf-8")
    (tmp_path / "bloom" / "acp" / "agent.py").write_text("", encoding="utf-8")
...
async def test_path_completion_popup_lists_files_and_directories(
    bloom_app: BloomApp, file_tree: Path
) -> None:
    async with bloom_app.run_test() as pilot:
        popup = bloom_app.query_one(CompletionPopup)
...
async def test_fuzzy_matches_subsequence_characters(
    file_tree: Path, bloom_app: BloomApp
) -> None:
    async with bloom_app.run_test() as pilot:
        popup = bloom_app.query_one(CompletionPopup)
...
async def test_finds_files_recursively_by_filename(
    ...
    async with bloom_app.run_test() as pilot:
        ...
        popup_content = str(popup.render())
        assert "@bloom/acp/entrypoint.py" in popup_content
        assert popup.styles.display == "block"
