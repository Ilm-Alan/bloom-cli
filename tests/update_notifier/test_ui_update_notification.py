...
from bloom.cli.textual_ui.widgets.messages import WhatsNewMessage
...
from tests.conftest import build_test_bloom_app, build_test_bloom_config
...
def build_update_test_app(
    bloom_config_with_update_checks_enabled: BloomConfig,
) -> Callable[..., BloomApp]:
    def _build(
        *,
        ...
        return build_test_bloom_app(
            ...
            config=config or bloom_config_with_update_checks_enabled,
...
async def _wait_for_notification(
    app: BloomApp, pilot, *, timeout: float = 1.0, interval: float = 0.05
) -> Notification:
...
@pytest.fixture
def bloom_config_with_update_checks_enabled() -> BloomConfig:
    return build_test_bloom_config(enable_update_checks=True)


@pytest.mark.asyncio
async def test_ui_displays_update_notification(
    build_update_test_app: Callable[..., BloomApp],
) -> None:
...
async def test_ui_does_not_invoke_gateway_nor_show_error_notification_when_update_checks_are_disabled(
    build_update_test_app: Callable[..., BloomApp], bloom_config: BloomConfig
) -> None:
    notifier = FakeUpdateGateway(update=Update(latest_version="0.2.0"))
    app = build_update_test_app(update_notifier=notifier, config=bloom_config)
...
async def test_ui_displays_whats_new_message_when_content_exists(
    build_update_test_app: Callable[..., BloomApp], tmp_path: Path
) -> None:
    notifier = FakeUpdateGateway(update=None)
    cache = UpdateCache(
...
async def test_ui_displays_success_notification_when_auto_update_succeeds(
    ...
    config = build_test_bloom_config(enable_update_checks=True, enable_auto_update=True)
    notifier = FakeUpdateGateway(update=Update(latest_version="0.2.0"))

    with patch("bloom.cli.update_notifier.update.UPDATE_COMMANDS", ["true"]):
...
async def test_ui_displays_update_notification_when_auto_update_fails(
    ...
    with patch("bloom.cli.update_notifier.update.UPDATE_COMMANDS", ["false"]):
        app = build_update_test_app(update_notifier=notifier, config=config)
