...
from bloom.cli.history_manager import HistoryManager
...
async def test_ui_resumes_arrow_down_after_manual_move(
    bloom_app: BloomApp, tmp_path: Path
) -> None:
    ...
    async with bloom_app.run_test() as pilot:
        inject_history_file(bloom_app, history_path)
        chat_input = bloom_app.query_one(ChatInputContainer)
        textarea = chat_input.input_widget
        assert textarea is not None
