...
from bloom.core.agents.models import (
    BUILTIN_AGENTS,
    PLAN_AGENT_TOOLS,
    AgentProfile,
    AgentSafety,
...
from bloom.core.paths.config_paths import ConfigPath
from bloom.core.paths.global_paths import GlobalPath
...
class TestAgentApplyToConfig:
    def test_custom_prompt_found_in_global_when_missing_from_project(
        ...
        """Regression test for https://github.com/pollinations/bloom-cli/issues/288
        ...
        ~/.bloom/prompts/ directory and load the file from there.
        """
        project_prompts = tmp_path / "project" / ".bloom" / "prompts"
        project_prompts.mkdir(parents=True)

        global_prompts = tmp_path / "home" / ".bloom" / "prompts"
        global_prompts.mkdir(parents=True)
        (global_prompts / "cc.md").write_text("Global custom prompt")

        monkeypatch.setattr(
            "bloom.core.config.PROMPTS_DIR", ConfigPath(lambda: project_prompts)
        )
        monkeypatch.setattr(
            "bloom.core.config.GLOBAL_PROMPTS_DIR", GlobalPath(lambda: global_prompts)
        )

        base = BloomConfig(include_project_context=False, include_prompt_detail=False)
        agent = AgentProfile(
            name="cc",
            display_name="Cc",
            description="",
...
class TestAgentManagerCycling:
    @pytest.fixture
    def base_config(self) -> BloomConfig:
    ...
    def test_get_agent_order_includes_primary_agents(
        self, base_config: BloomConfig, backend: FakeBackend
    ) -> None:
...
class TestAcceptEditsAgent:
    ...
    async def test_accept_edits_agent_auto_approves_write_file(self) -> None:
        ...
        config = build_test_bloom_config(
...
class TestPlanAgentToolRestriction:
    ...
    async def test_plan_agent_only_exposes_read_tools_to_llm(self) -> None:
        ...
        config = build_test_bloom_config(auto_compact_threshold=0)
        agent = build_test_agent_loop(
...
class TestAgentLoopInitialization:
    def test_agent_system_prompt_id_is_applied_on_init(
        ...
        monkeypatch.setattr("bloom.core.agents.models.BUILTIN_AGENTS", patched_agents)
        monkeypatch.setattr("bloom.core.agents.manager.BUILTIN_AGENTS", patched_agents)
