...
Mocked returns are stored in the BLOOM_MOCK_LLM_DATA environment variable.
"""
...
if __name__ == "__main__":
    ...
    from bloom.core.types import LLMChunk
    from tests import TESTS_ROOT
    from tests.mock.utils import MOCK_DATA_ENV_VAR

    sys.path.insert(0, str(TESTS_ROOT))

    # Apply mocking before importing any bloom modules
    ...
    patch(
        "bloom.core.llm.backend.generic.GenericBackend.complete",
        side_effect=mock_complete,
    ).start()
    patch(
        "bloom.core.llm.backend.generic.GenericBackend.complete_streaming",
        side_effect=mock_complete_streaming,
    ).start()

    from bloom.acp.entrypoint import main
