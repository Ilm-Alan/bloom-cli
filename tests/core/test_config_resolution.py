...
from bloom.core.paths.config_paths import CONFIG_FILE
from bloom.core.paths.global_paths import BLOOM_HOME, GLOBAL_CONFIG_FILE
...
class TestResolveConfigFile:
    def test_resolves_local_config_when_exists_and_folder_is_trusted(
        ...
        local_config_dir = tmp_path / ".bloom"
    ...
    def test_falls_back_to_global_config_when_local_missing(
        ...
        assert not (tmp_path / ".bloom" / "config.toml").exists()

        assert CONFIG_FILE.path == GLOBAL_CONFIG_FILE.path

    def test_respects_bloom_home_env_var(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        assert BLOOM_HOME.path != tmp_path
        monkeypatch.setenv("BLOOM_HOME", str(tmp_path))
        assert BLOOM_HOME.path == tmp_path
