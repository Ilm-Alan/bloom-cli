...
from bloom.setup.onboarding import OnboardingApp
from bloom.setup.onboarding.screens.api_key import _KEY_VALIDATION_URL, ApiKeyScreen
from bloom.setup.onboarding.screens.theme_selection import ThemeSelectionScreen
...
async def test_ui_gets_through_the_onboarding_successfully() -> None:
    respx.get(_KEY_VALIDATION_URL).mock(return_value=Response(200))
    ...
    async with app.run_test() as pilot:
        ...
        await pilot.press("enter")
        await _wait_for(lambda: app.return_value is not None, pilot, timeout=5.0)
    ...
    assert "POLLINATIONS_API_KEY" in env_contents
