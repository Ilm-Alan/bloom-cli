...
from bloom.core.agents.models import AgentProfile, AgentSafety
...
from bloom.core.types import AgentStats, LLMMessage, Role, SessionMetadata
...
@pytest.fixture
def mock_bloom_config() -> BloomConfig:
    """Create a mock bloom config for testing."""
    return build_test_bloom_config(active_model="test-model", models=[], providers=[])
...
class TestSessionLoggerMetadata:
    @patch("bloom.core.session.session_logger.subprocess.run")
    @patch("bloom.core.session.session_logger.getpass.getuser")
...
class TestSessionLoggerSaveInteraction:
    ...
    async def test_save_interaction_disabled(
        ...
        result = await logger.save_interaction(
            messages=[],
            stats=AgentStats(),
            base_config=build_test_bloom_config(
