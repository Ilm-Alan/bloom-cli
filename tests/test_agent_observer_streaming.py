...
from bloom.core.llm.exceptions import BackendErrorBuilder
...
from bloom.core.tools.builtins.todo import TodoArgs
...
async def test_act_flushes_batched_messages_with_injection_middleware(
    ...
    assert observed[0][1] == "You are Bloom, a super useful programming assistant."
...
async def test_act_streams_batched_chunks_in_order() -> None:
    ...
    assert [event.content for event in assistant_events] == [
        "Hello from Bloom! More",
        " and end",
    ]
    assert agent.messages[-1].role == Role.assistant
    assert agent.messages[-1].content == "Hello from Bloom! More and end"
